...
function saveRecord(){
    ...
    if(isValid == true){
        ...
        formData.append('type','prestacion');
        ...
        let cont_files=0;
        document.querySelectorAll(".prestacion_files").forEach((temp_input,key) =>{
            formData.append(`files_${key}`,temp_input.files[0]);
            cont_files++;
        })
        formData.append('cont_files',cont_files);

        axios.post('/admin/benefits/prestacion-save',formData,config)
        .then(response =>{
...
function editRecord(_id){
    ...
    document.querySelector("#prestacion_id").value= element.id;
...
async function updateRecord(){
    ...
    if(isValid == true){
        ...
        formData.append('prestacion', document.querySelector('#prestacion_id').value);
        ...
        await axios.post('/admin/benefits/prestacion-edit',formData,config)
        .then(response =>{
...
function deleteRecord(_id){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get('/admin/benefits/prestacion-delete/'+_id)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
    ...
    let row_HTML = `
        <div class="col-md-12 mb-3" id="div_file_${element_id}">
            <div class="input-file" id="input-file_${element_id}">
                ...
                <input class="form-control d-none prestacion_files" type="file" accept="application/.xls,.doc,.xlsx,.docx,.pdf" id="${element_id}" placeholder="Imagen" onchange="uploadDocument(this)" required>
            </div>
        </div>
    `;
...
function deleteUploadedDoc(_event){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get(`/admin/benefits/prestacion-delete-file/${file_id}/${record_id}`)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
