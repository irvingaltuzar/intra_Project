...
var general_data = {
    ...
    fields_modal:{
        required:['select_location','txt_expiration_date','txt_title','file_photo'],
        edit_required:['select_location_edit','txt_expiration_date_edit','txt_title_edit'],
        total:['select_location','select_subgroup','txt_expiration_date','txt_title','txt_link','txt_description','file_photo','file_video'],
    },
    locations:{},
    subgroups:{}
...
function showList(val) {
    ...
    .then(response =>{
        ...
        val == 0 ? $('#modal_user_list_area_notice').modal('show') : $('#modal_user_list_area_notice_edit').modal('show')
    })
    .catch(error =>{
...
function sendReminder(_id){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get('/admin/news/area-notice/send-reminder/'+_id)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
...
function saveRecord(){
    ...
    if(isValid == true){
        ...
        let cont_files=0;
        document.querySelectorAll(".area_notice_files").forEach((temp_input,key) =>{
            formData.append(`files_${key}`,temp_input.files[0]);
            cont_files++;
        })
        formData.append('cont_files',cont_files);

        axios.post('/admin/news/area-notice-save',formData,config)
        .then(response =>{
...
function validateVideo(file){
    ...
    if(((file.size/1024)/1024) < 1500){
        ...
        if(file.type == "video/mp4"){
            console.log('peso de video',(file.size/1024)/1024);
            return true;
        }else{
...
function viewRecord(_id){
    ...
    if(element.video != null){
        document.querySelector("#file_video_view").src= ths.url_base+'/'+element.video;
        document.querySelector("#file_video_view").classList.remove('d-none');
    }else{
...
function editRecord(_id){
    ...
    document.querySelector("#area_notice_id").value= element.id;
    ...
    document.querySelector("#file_video_view_edit").src= ths.url_base+'/'+element.video;
...
async function updateRecord(){
    ...
    if(isValid == true){
        ...
        formData.append('area_notice', document.querySelector('#area_notice_id').value);
        ...
        await axios.post('/admin/news/area-notice-edit',formData,config)
        .then(response =>{
...
function deleteRecord(_id){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get('/admin/news/area-notice-delete/'+_id)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
    ...
    let row_HTML = `
        <div class="col-md-12 mb-3" id="div_file_${element_id}">
            <div class="input-file" id="input-file_${element_id}">
                ...
                <input class="form-control d-none area_notice_files" type="file" accept="application/.xls,.doc,.xlsx,.docx,.pdf" id="${element_id}" placeholder="Imagen" onchange="uploadDocument(this)" required>
            </div>
        </div>
    `;
...
function deleteUploadedDoc(_event){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get(`/admin/news/area-notice-delete-file/${file_id}/${record_id}`)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
