...
var general_data = {
    ...
    fields_modal:{
        required:['select_location','txt_publication_date','txt_expiration_date','txt_title','file_photo'],
        edit_required:['select_location_edit','txt_publication_date_edit','txt_expiration_date_edit','txt_title_edit'],
        total:['select_location','select_subgroup','txt_publication_date','txt_expiration_date','txt_title','txt_description','file_photo'],
    },
    locations:{},
    subgroups:{},
...
function saveRecord(){
    ...
    if(isValid == true){
        ...
        formData.append('publication_date', document.querySelector('#txt_publication_date').value);
        formData.append('expiration_date', document.querySelector('#txt_expiration_date').value);

        //Se recogen los documentos cargados
        let cont_files=0;
        document.querySelectorAll(".internal_posting_files").forEach((temp_input,key) =>{
        ...
        axios.post('/admin/news/internal-posting-save',formData,config)
        .then(response =>{
...
function viewRecord(_id){
    ...
    document.querySelector("#txt_publication_date_view").value= element.publication_date;
...
function sendReminder(_id){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get('/admin/news/internal-posting-delete/send-reminder/'+_id)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
...
function editRecord(_id){
    ...
    document.querySelector("#internal_posting_id").value= element.id;
    ...
    document.querySelector("#txt_publication_date_edit").value= element.publication_date;
...
async function updateRecord(){
    ...
    if(isValid == true){
        ...
        formData.append('internal_posting', document.querySelector('#internal_posting_id').value);
        ...
        formData.append('publication_date', document.querySelector('#txt_publication_date_edit').value);
        ...
        await axios.post('/admin/news/internal-posting-edit',formData,config)
        .then(response =>{
...
function deleteRecord(_id){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get('/admin/news/internal-posting-delete/'+_id)
            .then(response =>{
                let resp = response.data;
                if(resp.success == 1){
                    ths.getPage(1);
    ...
    let row_HTML = `
        <div class="col-md-12 mb-3" id="div_file_${element_id}">
            <div class="input-file" id="input-file_${element_id}">
                ...
                <input class="form-control d-none internal_posting_files" type="file" accept="application/.xls,.doc,.xlsx,.docx,.pdf" id="${element_id}" placeholder="Imagen" onchange="uploadDocument(this)" required>
            </div>
        </div>
    `;
