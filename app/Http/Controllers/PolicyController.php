...
class PolicyController extends Controller
{
    ...
    public function policyShow(){
        ...
        $list = $this->adminPolicyList($temp_request);
        $submodule = "policy";
        ...
        return view('admin.news.policy.list')->with($compact);

    }

    public function publicPolicyList(Request $request){

        $limit = (isset($request->limit) && $request->limit > 0) ? $request->limit : '10';
        $policy_list = Policy::join('bucket_locations','policies.id','bucket_locations.origin_record_id')
        ...
        ->whereDate('policies.expiration_date', '>=', Carbon::now())
        ->orderBy('policies.created_at','desc')
        ->select('policies.*')
        ->Paginate($limit);

        $policy_list->setPath('/news/policy');

        return $policy_list;
    }


    public function adminPolicyList(Request $request){
        ...
        if(isset($search->isSearch) && strlen($search->text) > 0){
            $policy_list = Policy::whereRaw("(title like '%$search->text%'
                or description like '%$search->text%'
                or expiration_date like '%$search->text%')")
            ->orderBy('expiration_date',$order_by)->Paginate($limit);
        }else{
            $policy_list = Policy::orderBy('expiration_date',$order_by)
            ->Paginate($limit);
        }

        $policy_list->setPath('/admin/news/policy-list');
    ...
    public function policyEdit(Request $request){
        ...
        if($editRecord != null){
            ...
            $locations_old = BucketLocation::where('origin_record_id',$request->policy)
    ...
    public function policyDelete($_id){

        if(isset($_id) && $_id > 0){
            $record = Policy::where('id',$_id)->first();
            if($record != null){
    ...
    public function sendEmails($_record_id,$_sub_seccion_id){
        ...
        if($record != null){
            ...
            $mail_data = [
                ...
                'subject' => "Políticas - $record->title",
